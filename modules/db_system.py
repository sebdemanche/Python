...
def stop_database_systems(config, signer, compartments ,tagdata):
    target_resources = []
    ...
    for compartment in compartments:
        ...
        for db_system in db_systems:
            ...
            if (db_system.lifecycle_state == 'AVAILABLE'):
                if (tagdata['TagNamespace'] in db_system.defined_tags) and (tagdata['TagKey'] in db_system.defined_tags[tagdata['TagNamespace']]): 
                    if (db_system.defined_tags[tagdata['TagNamespace']][tagdata['TagKey']].upper() != tagdata['TagValue']):
                        go = 1
                else:
                    go = 1
